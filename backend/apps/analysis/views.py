...
from .utils import integrate_advanced_statistics, generate_plot_file, generate_csv_file
...
def process_landsat_ndvi_analysis(geometry, start_date, end_date):
    ...
    try:
        ...
        if earth_engine_utils:
            ...
            if not collections:
                raise Exception(
                    "Landsat NDVI requires data from 2013 onwards (Landsat 8/9)"
                )
            ...
            try:
                ...
                if collection_size == 0:
                    raise Exception(
                        f"No Landsat 8/9 images found for period {start_date} to {end_date}"
                    )
            except Exception as e:
        ...
        try:
            sample_points = ndvi_band.sample(
                region=geometry, scale=30, numPixels=100
            ).getInfo()
...
def process_lst_analysis(geometry, start_date, end_date, satellite="modis"):
    ...
    try:
        logger.info(
            f"Processing LST analysis for {start_date} to {end_date} using {satellite}"
        )
        ...
        if satellite.lower() == "landsat":
            ...
            raise Exception(
                f"Unsupported satellite for LST analysis: {satellite}. Supported: landsat, modis"
            )
    ...
    try:
        ...
        try:
            ...
            if end_year >= 2013:
                ...
                collections.append(l8)
                logger.info("Added Landsat 8 collection for LST")
            ...
            if end_year >= 2021:
                ...
                collections.append(l9)
                logger.info("Added Landsat 9 collection for LST")

            if not collections:
                raise Exception(
                    "Landsat LST requires data from 2013 onwards (Landsat 8/9 with thermal bands)"
                )
            ...
            if collection_size == 0:
                raise Exception(
                    f"No Landsat images found for LST analysis in period {start_date} to {end_date}"
                )
                ...
                if result.get("demo_mode") is False and (
                    ...
                    if stats:
                        ...
                        for i in range(num_points):
                            ...
                            # Use secure random for synthetic data generation
                            value = mean_val + (
                                secrets.randbelow(int(std_val * 2 * 1000)) / 1000.0
                                - std_val
                            )
