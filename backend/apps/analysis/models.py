...
class AnalysisRequest(models.Model):
    ...
    project = models.ForeignKey(AnalysisProject, on_delete=models.CASCADE, related_name='analyses', null=True, blank=True)
    
    # Analysis parameters
    name = models.CharField(max_length=255)
    analysis_type = models.CharField(max_length=20, choices=ANALYSIS_TYPE_CHOICES)
    satellite = models.CharField(max_length=20, choices=SATELLITE_CHOICES, default='landsat')
    ...
    selected_images = models.JSONField(null=True, blank=True)  # Store selected image indices
    ...
    polarization = models.CharField(max_length=10, choices=POLARIZATION_CHOICES, default='VV', null=True, blank=True)
...
class AnalysisResult(models.Model):
    ...
    # Generated files
    plot_file = models.FileField(upload_to='results/plots/%Y/%m/%d/', null=True, blank=True)
    csv_file = models.FileField(upload_to='results/csv/%Y/%m/%d/', null=True, blank=True)
    map_file = models.FileField(upload_to='results/maps/%Y/%m/%d/', null=True, blank=True)
...
class ImageCollection(models.Model):
    ...
    analysis_request = models.ForeignKey(AnalysisRequest, on_delete=models.CASCADE, related_name='image_collections')
