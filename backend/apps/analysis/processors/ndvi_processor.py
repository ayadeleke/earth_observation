...
class NDVIProcessor:
    ...
    def ensure_earth_engine_initialized(self):
        ...
        try:
            ...
            if (
                hasattr(settings, "EARTH_ENGINE_SERVICE_ACCOUNT_KEY")
                and settings.EARTH_ENGINE_SERVICE_ACCOUNT_KEY
            ):
    ...
    def process_landsat_ndvi(
        ...
        try:
            logger.info(
                f"Starting NDVI processing for date range: {start_date} to {end_date}"
            )
            logger.info(f"Cloud cover threshold: {cloud_cover}%")
            logger.info(
                f"Cloud masking enabled: {use_cloud_masking} (strict: {strict_masking})"
            )
            ...
            ndvi_features = ndvi_collection.map(
                lambda image: self._reduce_region_ndvi(
                    image, ee_geometry, use_cloud_masking
                )
            )
                    ...
                    records.append(
                        {
                            ...
                            "ndvi": props["ndvi"],
                            "original_cloud_cover": props.get(
                                "original_cloud_cover", "N/A"
                            ),
                            "effective_cloud_cover": props.get(
                                "effective_cloud_cover", "N/A"
                            ),
                            "cloud_masking_applied": props.get(
    ...
    def _get_cloud_masked_collection(
        self,
        start_date,
        end_date,
        geometry,
        ...
        # Get base collection
        collection = self._get_landsat_collection(
            start_date, end_date, geometry, cloud_cover, require_complete_coverage
        )
    ...
    def _reduce_region_ndvi(self, image, geometry, use_cloud_masking):
        ...
        if use_cloud_masking:
            ...
            data_availability = (
                valid_count.divide(total_count.max(1)).multiply(100).min(100).max(0)
            )
            effective_cloud_cover = (
                ee.Number(100).subtract(data_availability).max(0).min(100)
            )
        else:
