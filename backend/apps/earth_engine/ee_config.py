...
def initialize_earth_engine():
    ...
    try:
        ...
        service_account_key_path = getattr(settings, 'EARTH_ENGINE_SERVICE_ACCOUNT_KEY', None)
        ...
        if use_service_account and service_account_key_path:
            ...
            if key_file_path.exists():
                ...
                logger.info(f"[SUCCESS] Earth Engine initialized with service account for project: {project_id}")
                _ee_initialized = True
                return True
            else:
                logger.warning(f"[WARNING] Service account key file not found: {key_file_path}")
                logger.info("Falling back to default authentication...")
        
        # Fallback to default authentication (for development)
        try:
            ee.Initialize(project=project_id)
            logger.info(f"[SUCCESS] Earth Engine initialized with default authentication for project: {project_id}")
            _ee_initialized = True
            return True
        except Exception as fallback_error:
            logger.error(f"[ERROR] Default authentication failed: {str(fallback_error)}")
...
def get_authentication_info():
    ...
    auth_method = "Service Account" if (use_service_account and service_account_available) else "Default/Demo"
